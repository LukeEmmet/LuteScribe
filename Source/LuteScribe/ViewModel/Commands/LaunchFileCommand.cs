...
namespace LuteScribe.ViewModel.Commands
{
    ...
    public class LaunchFileCommand : ICommand
    {
        ...
        {
            //assumes passed in a relative path
            //start file in its default application
            var path = (string)parameter;
            var isAbsolute = Path.IsPathRooted(path);

            if (!isAbsolute)
            {
                //convert to a full path wrt current app folder.
                path = Path.GetFullPath(System.AppDomain.CurrentDomain.BaseDirectory + path);
            }
